...
class BO:
    ...
    def __call__(self, acquisition: Callable, xi: float, n_iter: int, name: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(n_iter):
            ...
            Y_next = _f(X_next) + _noise_level * np.eye(X_next.shape[0])


            temp_X_next = copy.deepcopy(self._X_sample).tolist()
            for x in X_next:
                temp_X_next.append(x.tolist())
            temp_Y_next = copy.deepcopy(self._Y_sample).tolist()
            for y in Y_next:
                temp_Y_next.append(y.tolist())

            self._X_sample = np.array(temp_X_next)
            self._Y_sample = np.array(temp_Y_next)
        ...
        return self._X_sample,self._Y_sample
