...
class GPRegressor:
    ...
    def optimisation(self,
        ...
        # FIXME
        res = minimize(obj_func,initial_theta,method='L-BFGS-B',bounds = bounds)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> GPRegressor:
        ...
        for i in range(self.n_restarts):
            ...
            for bound in bounds:
                theta = self.random_state.uniform(bound[0], bound[1])
                temp_thetas.append(theta)

            temp_thetas = np.array(temp_thetas)
            self._kernel.theta = temp_thetas

            fun = self.negative_log_marginal_likelihood
            theta,fun = self.optimisation(fun,temp_thetas,bounds)
            if fun < min_fun:
                min_fun = fun
                best_theta = theta

        self._kernel.theta = best_theta
    ...
    def predict(self, X: np.ndarray, return_std: bool = False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        Ky = kernel(self._X_train)
        Ky = Ky + self.noise_level*np.eye(Ky.shape[0])
        K_ = kernel(self._X_train,X)
        K_T = K_.T
        K__ = kernel(X)

        L = cholesky(Ky,lower=True)
        x = cho_solve((L,True),self._y_train)
        y_mean = np.dot(K_T,x)

        x = cho_solve((L,True),K_)
        cov = K__-np.dot(K_T,x)

        y_std = np.sqrt(np.diag(cov)).reshape(-1,1)
        if return_std:
            return y_mean,y_std
        else:
            return y_mean
    ...
    def fit_and_predict(self, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray,
        ...
        if optimise_fit:
            ...
            self._kernel = copy.deepcopy(self.kernel)
            params = self._kernel.get_params()

            if return_std:
                y_mean, y_std = self.predict(X_test, return_std=True)
                return params, (y_mean, y_std)
            else:
                y_mean = self.predict(X_test)
                return params, y_mean
        else:
            self._kernel = copy.deepcopy(self.kernel)
    ...
    def log_marginal_likelihood(self, theta: np.ndarray) -> float:
        ...
        x1 = (-1/2)*np.dot(self._y_train.T,x).ravel()[0]
        x2 = (-1/2)*np.linalg.slogdet(Ky)[1].ravel()[0]
        n = len(self._X_train)
        x3 = (-n/2)*np.log(2*np.pi)
        output = x1 + x2 + x3
        return output

    def negative_log_marginal_likelihood(self, theta: np.ndarray):
        kernel = self._kernel
        kernel.theta = theta
        Ky = kernel(self._X_train)
        Ky = Ky + self.noise_level * np.eye(Ky.shape[0])
