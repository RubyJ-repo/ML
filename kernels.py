...
class Matern(Kernel):
    ...
    def __call__(self, X: np.ndarray, Y: np.ndarray = None) -> np.ndarray:
        ...
        if self.nu == 1.5:
            # FIXME
            output = self.variance*(1+np.sqrt(3)*d/length_scale)*np.exp(-np.sqrt(3)*d/length_scale)
            return output
        elif self.nu == 2.5:
            # FIXME
            output = self.variance*(1 + np.sqrt(5) * d / length_scale + 5*(d**2)/(3*length_scale**2)) * np.exp(-np.sqrt(5) * d / length_scale)
            return output
        else:
            # Do not change
